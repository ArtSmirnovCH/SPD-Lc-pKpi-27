...
void sv_reconstruction(Int_t N, std::string_view tag, Int_t SEED) // N - max event number to analyse
{
	...
	tree -> Branch("dctau_Lc", &dctau_Lc, "dctau_Lc/D");
	...
	tree -> Branch("probab_p", &probab_p, "probab_p/D");
	tree -> Branch("probab_k", &probab_k, "probab_k/D");
	tree -> Branch("probab_pip", &probab_pip, "probab_pip/D");
    ...
    while ( IT -> NextEvent() && n_event < events_max ) 
	{
		...
		while ( vfinder -> Next(vc) ) 					// Each iteration takes new particles(tracks) combination (of 3 particles for the current decay)
		{
			...
			{ 
				...
				{
					i_p = i;
					probab_p = softmax_outputs_tof[vc[i]][2];
				}
				if ( pdgs[vc[i]] == -321 )
				{
					i_K = i;
					probab_k = softmax_outputs_tof[vc[i]][1];
				}
				if ( pdgs[vc[i]] == 211 )
				{
					i_pip = i;
					probab_pip = softmax_outputs_tof[vc[i]][0];
				}
			} 
			//============================================================================================
			// Tracks selection
			...
			Double_t temp_Lc_z_coord{ Lc_position.Z() };
			...
			//==================================================================================
			// Momentum direction check (XY - plane])
            Lc.TransportToProductionVertex();
			...
			Lc_diff_z = temp_Lc_z_coord - Lc_true_position.Z();
			//==================================================================================
			// Check cuts + Cut Flow
...
// Launcher function
//======================================================================================================================
void analyze_Lc_pKpi_27( Bool_t signal = 1, Int_t SEED = 0, Int_t N_max = 500 ) 
{   
