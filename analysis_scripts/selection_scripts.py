...
def auto_preselection(df: pd.DataFrame,
                      ...
                      quantile_dict: dict = None) -> Tuple[pd.DataFrame, str]:
    ...
    Args:
        ...
        - quantile_left (float, optional): Lower quantile threshold for outlier removal.
            Data below this quantile will be considered outliers.
        - quantile_right (float, optional): Upper quantile threshold for outlier removal.
            Data above this quantile will be considered outliers.
        - recursive (bool, optional): If True, applies outlier removal sequentially feature-by-feature,
            ...
            If False, calculates all cuts on the original dataset.
        - quantile_dict (dict): set specific left and right quantiles for choosen features.(features: (left , right)) 
    ...
    for feature in features:
        ...
        if feature in quantile_dict:
            left_cut = df[feature].quantile(quantile_dict[feature][0])
            right_cut = df[feature].quantile(quantile_dict[feature][1])    
        else:
...
def find_optimal_cut_point(
    ...
    if metric_type not in ['tpr_fpr', 'f1', 's_over_b', 's_over_b_mod']:
        raise ValueError(f"Invalid metric_type: {metric_type}. \n Should be 'tpr_fpr', 'f1', 's_over_b' or 's_over_b_mod'.")
    ...
    for num, cut_x in enumerate(thresholds):
        ...
        if metric_type == 'tpr_fpr':
        ...
        elif metric_type == 's_over_b_mod' or metric_type == 's_over_b':
            
            if select_direction == 'right':
                sig_mass_distr = df.loc[(df.tag == 'Sig') & (df[feature] > cut_x), 'mass_Lc']
                bg_mass_distr = df.loc[(df.tag == 'Bg') & (df[feature] > cut_x), 'mass_Lc']
                sig_eff = np.sum(distr_sig > cut_x) / np.sum(~np.isnan(distr_sig))
                bg_eff = np.sum(distr_bg > cut_x) / np.sum(~np.isnan(distr_bg))
            ...
            if not len(sig_mass_distr) or not len(bg_mass_distr):
                metric_val = 0
            else:
                sig_weight = CFG.real_sig_events / CFG.have_sig_events_2
                bg_weight = CFG.real_bg_events / CFG.have_bg_events_2
                if metric_type == 's_over_b':
                    metric_val = sig_weight / bg_weight * len(sig_mass_distr) / len(bg_mass_distr)
                else:
                    metric_val = sig_weight / bg_weight * len(sig_mass_distr) / len(bg_mass_distr) * sig_eff * (1 - bg_eff)
...
def calculate_feature_importance(
    ...
    metric_type_cut_point: str = 'tpr_fpr',       # Respond only for cut point selection, feature importance ranking controls by AUC
    metric_type_feature_ranking: str = 'AUC', # feature importance ranking (probably better to use s_over_b here if it's used in metric_type_cut_point)
    mass_interval: Tuple[float, float] = (2.24763, 2.32497),
    nbins: int = 100,
) -> pd.DataFrame:
    
    if metric_type_cut_point not in ['tpr_fpr', 'f1', 's_over_b', 's_over_b_mod']:
        raise ValueError(f"Invalid metric_type_cut_point: {metric_type_cut_point}. \n Must be one of: 'tpr_fpr', 'f1', 's_over_b', 's_over_b_mod'")
    
    if metric_type_feature_ranking not in ['AUC', 's_over_b', 's_over_b_mod']:
            raise ValueError(f"Invalid metric_type_feature_ranking: {metric_type_feature_ranking}. \n Must be one of: 'AUC', 's_over_b', 's_over_b_mod'")
    ...
    for num, feature in enumerate(features):
        ...
        for direction in ['left', 'right']:
            ...
            min_sig_sel_mask, best_arg, metric, thresholds, tpr, fpr, sig_efficiency, bg_efficiency = find_optimal_cut_point(
                ...
                metric_type=metric_type_cut_point,
            ...
            if metric_type_feature_ranking == 'AUC':
                metric_value = auc(fpr, tpr) # Ignors min_sig_sel_mask; This is another metric used for feature importance ranking
            elif metric_type_feature_ranking == 's_over_b_mod' or metric_type_feature_ranking == 's_over_b':
                metric_value = metric[min_sig_sel_mask][best_arg]    # Get metric from cut criteria
            
            print(f'{metric_value=:.4f}, direction: {direction}')
    ...
    importance_df = pd.DataFrame({
        f'metric_value': metric_value_list,
    ...
    importance_df = importance_df.sort_values(by=f'metric_value', ascending=False).reset_index(drop=True)
...
def create_best_selection_path(
    df: pd.DataFrame, 
    features: List[str], 
    metric_type_cut_point: str, 
    metric_type_feature_ranking: str, # [tpr-fpr, AUC], [s_over_b, s_over_b]
    have_sig_events: int,
    have_bg_events: int, 
    ...
    if metric_type_cut_point not in ['tpr_fpr', 'f1', 's_over_b', 's_over_b_mod']:
        raise ValueError(
            f"Invalid metric_type_cut_point: {metric_type_cut_point}. \n Must be one of: 'tpr_fpr', 'f1', 's_over_b', 's_over_b_mod'")
        
    if metric_type_feature_ranking not in ['AUC', 's_over_b', 's_over_b_mod']:
            raise ValueError(
                f"Invalid metric_type_feature_ranking: {metric_type_feature_ranking}. \n Must be one of: 'AUC', 's_over_b','s_over_b_mod'")
    ...
    for i in range(num_iter):
        ...
        # Signal and background distributions before selection
        ...
        try:  
            importance_df = calculate_feature_importance(
                ...
                metric_type_cut_point=metric_type_cut_point,
                metric_type_feature_ranking=metric_type_feature_ranking,
                mass_interval=mass_interval,
                nbins=nbins,
            )
        ...
        # Signal and background distributions after selection
        sig_mass_distr = df.loc[(df['tag'] == 'Sig'), 'mass_Lc'].copy()
        bg_mass_distr = df.loc[(df['tag'] == 'Bg'), 'mass_Lc'].copy()
